...
def josephus_task(num_people, kill_num):
    index_num = kill_num
    surviver_list = list(range(1,num_people + 1))
    while index_num > len(surviver_list):
        index_num -= len(surviver_list)
    while len(surviver_list) != 1:
        del surviver_list[index_num - 1]
        index_num += kill_num - 1
        while index_num > len(surviver_list):
            index_num -= len(surviver_list)
    surviver = surviver_list[0]
    return surviver
